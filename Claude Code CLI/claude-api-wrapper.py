...
app = FastAPI(
    ...
    description="HTTP API обертка для Claude Code",
    version="1.0.0"
)
...
class ClaudeCodeInterface:
    ...
    async def chat(cls, message: str, session_id: str, model: str = "sonnet") -> str:
        ...
        # ВАЖНО: --allowedTools разрешает WebSearch без интерактивного подтверждения
        # (безопаснее чем --dangerously-skip-permissions, который не работает под root)
