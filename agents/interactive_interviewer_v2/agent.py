...
# Since we're now in agents/interactive_interviewer_v2/agent.py
...
# Reference Points Framework (relative import within subproject)
...
class InteractiveInterviewerAgentV2(BaseAgent):
    ...
    async def conduct_interview(
        ...
        # ITERATION 53 FIX: НЕ запускаем аудит автоматически!
        # Аудит будет запущен только когда пользователь нажмёт кнопку "Начать аудит"
        # Это позволяет быстро финализировать интервью и отправить анкету пользователю
        logger.info("\n[FINALIZE] Интервью завершено, анкета собрана")
        logger.info("[FINALIZE] Аудит будет запущен когда пользователь нажмёт кнопку")

        processing_time = time.time() - start_time

        logger.info("=" * 80)
        logger.info(f"ИНТЕРВЬЮ ЗАВЕРШЕНО | Вопросов задано: {self.flow_manager.context.questions_asked} | "
    ...
    async def _send_greeting(
        ...
        # Greeting is sent by handler, skip it here
        # Приветствие уже отправлено handler'ом, пропускаем
        logger.info("[GREETING] Skipping greeting - already sent by handler")
        pass
    ...
    async def _save_to_db(
        ...
        Raises:
            NotImplementedError: Database save not yet implemented
        """
