...
def get_user_progress(telegram_id: int) -> Dict:
    ...
    """
    # Создаем экземпляр БД с параметрами по умолчанию из окружения
    ...
    with db.connect() as conn:
        ...
        for interview_data, collected_data, status in sessions:
            # Парсим interview_data (JSONB уже dict в PostgreSQL)
            if interview_data:
                if isinstance(interview_data, dict):
                    all_answers.update(interview_data)
            ...
            # Парсим collected_data (JSONB уже dict в PostgreSQL)
            if collected_data:
                if isinstance(collected_data, dict):
                    all_answers.update(collected_data)
                elif isinstance(collected_data, str):
                    try:
