-- ============================================================================
-- Migration: Add Expert Agent Knowledge Base Tables (No Vectors - для Qdrant)
-- ============================================================================
-- Migration Number: 012 (Modified for Qdrant)
-- Author: grant-architect agent
-- Date: 2025-10-17
-- Purpose: Add tables for Expert Agent (без knowledge_embeddings - векторы в Qdrant)
-- Database: grantservice (existing)
-- Dependencies: None (pgvector не нужен! Векторы теперь в Qdrant)
-- ============================================================================
--
-- ВАЖНО: Эта версия миграции НЕ создает таблицу knowledge_embeddings
-- Векторные embeddings теперь хранятся в Qdrant (векторная БД)
--
-- Архитектура:
--   PostgreSQL: структурированные данные (sources, sections, criteria, examples)
--   Qdrant:     векторные embeddings + metadata для поиска
-- ============================================================================

BEGIN;

-- ============================================================================
-- STEP 1: Create knowledge_sources table
-- ============================================================================
...
-- ============================================================================
-- STEP 2: Create knowledge_sections table
-- ============================================================================
...
-- Full-text search index (для текстового поиска в дополнение к векторному)
...
-- ============================================================================
-- STEP 3: Create successful_grant_examples table
-- ============================================================================
...
-- ============================================================================
-- STEP 4: Create evaluation_criteria table
-- ============================================================================
...
-- ============================================================================
-- STEP 5: Create knowledge_updates table (audit log)
-- ============================================================================
...
-- ============================================================================
-- STEP 6: Create views for convenience
-- ============================================================================

-- View: Active knowledge (без embeddings - они в Qdrant!)
CREATE OR REPLACE VIEW v_expert_active_knowledge AS
SELECT
    ks.id,
    src.fund_name,                             -- ИСПРАВЛЕНО: fund_name из knowledge_sources!
    ks.section_type,
    ks.section_name,
    ks.content,
    ks.char_limit,
...
COMMENT ON VIEW v_expert_active_knowledge IS 'Expert Agent: Актуальные знания (embeddings в Qdrant)';
...
-- ============================================================================
-- STEP 8: Add Expert Agent to ai_agent_settings (if table exists)
-- ============================================================================
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'ai_agent_settings') THEN
        ...
        VALUES ('expert', 'active', 'claude_code',
                '{"temperature": 0.5, "embedding_model": "rubert", "vector_db": "qdrant", "vector_search_threshold": 0.75}'::jsonb)
        ON CONFLICT (agent_name) DO NOTHING;

        RAISE NOTICE 'Expert agent added to ai_agent_settings (with Qdrant config)';
    ELSE
...
BEGIN
    ...
    RAISE NOTICE '(Modified for Qdrant - No Vectors Version)';
    RAISE NOTICE '============================================';
    ...
    RAISE NOTICE '============================================';
    RAISE NOTICE 'Vector storage: Qdrant (not PostgreSQL!)';
    RAISE NOTICE 'Architecture: PostgreSQL + Qdrant hybrid';
    RAISE NOTICE 'Next step: Install Qdrant and create collections';
    RAISE NOTICE '============================================';
END $$;
...
-- ============================================================================
-- END OF MIGRATION 012 (No Vectors Version)
-- ============================================================================
--
-- Что УБРАНО из оригинальной миграции:
--   ❌ CREATE EXTENSION vector (pgvector не нужен!)
--   ❌ CREATE TABLE knowledge_embeddings (векторы теперь в Qdrant)
--   ❌ HNSW индексы на векторах (в Qdrant автоматически)
--   ❌ Функция expert_search_similar_sections() (поиск теперь через Qdrant API)
--
-- Что ДОБАВЛЕНО:
--   ✅ Комментарии о Qdrant
--   ✅ Config для Qdrant в ai_agent_settings
--
-- Следующий шаг:
--   1. Запустить Qdrant: docker run -p 6333:6333 qdrant/qdrant
--   2. Создать collection в Qdrant для knowledge_sections
--   3. Реализовать Python модуль для синхронизации PostgreSQL ↔ Qdrant
-- ============================================================================
