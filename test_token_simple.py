...
# -*- coding: utf-8 -*-
"""
Тестовый скрипт для генерации токена для конкретного пользователя (без эмодзи)
"""
...
def test_token_generation():
    ...
    with db.connect() as conn:
        ...
        if result and result[0] == token:
            print("   [OK] Токен успешно сохранен в базе данных")
        else:
            print("   [ERROR] Токен НЕ сохранен в базе данных")
    ...
    if user_data:
        print("   [OK] Токен успешно прошел валидацию")
        print(f"   Данные пользователя: {user_data}")
    else:
        print("   [ERROR] Токен НЕ прошел валидацию")
